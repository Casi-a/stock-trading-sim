...
double Account::getInitialEquity() const {
    return initialEquity;
}

const std::vector<Position*>& Account::getPositions() const {
    return positions;
}

// A팀 명세 누락 메서드 구현: 타입별 포지션 조회
std::vector<Position*> Account::getPositionsByType(InstrumentType type) const {
